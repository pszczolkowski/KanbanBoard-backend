...
@RestValidator
public class ColumnDeleteValidator extends AbstractValidator {

	private final BoardSnapshotFinder boardSnapshotFinder;
	private final ColumnSnapshotFinder columnSnapshotFinder;
	
	@Autowired
	public ColumnDeleteValidator(BoardSnapshotFinder boardSnapshotFinder, ColumnSnapshotFinder columnSnapshotFinder) {
	...
	public boolean supports(Class<?> clazz) {
		return ColumnDelete.class.isAssignableFrom(clazz);
	}

	@Override
	public void customValidation(Object target, Errors errors) {
		ColumnDelete columnDelete = (ColumnDelete) target;
		
		ColumnSnapshot columnSnapshot = columnSnapshotFinder.findById(columnDelete.getColumnId());
		...
		if (columnSnapshot.getTasks().size() > 0) {
			if (columnDelete.getColumnToMove() == null) {
				errors.rejectValue("columnToMove", "MissingColumnToMove");
				return;
			}
			
			ColumnSnapshot columnToMoveSnapshot = columnSnapshotFinder.findById(columnDelete.getColumnToMove());
			if (columnToMoveSnapshot == null || columnToMoveSnapshot.getBoardId() != columnSnapshot.getBoardId()) {
				errors.rejectValue("columnToMove", "MissingColumnToMove");
				return;
			}
		}
